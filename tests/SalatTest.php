...
use Alhoqbani\ArPHP\Salat;

class SalatTest extends TestCase
{
    ...
    /** @test */
    public function it_returns_the_correct_paryers_time()
    {
        ...
        $this->assertEquals(
            $expected_times,
            $times,
            'The return prayres times must match the expected pryers time for
                    date, location and configs defined by the user'
        );
    }
    
    /** @test */
    public function it_returns_paryers_time_for_user_defined_configration()
    {
        $this->salat->setConf('Shafi', -0.833333, -17.5, -19.5, 'Sunni');
        $times = $this->salat->getPrayTime();
    ...
    /** @test */
    public function it_returns_paryers_time_for_user_defined_location()
    {
        $this->salat->setLocation(33.52, 36.31, 3, 691);
        $times = $this->salat->getPrayTime();
    ...
    /** @test */
    public function it_returns_paryers_time_for_user_defined_date()
    {
        $this->salat->setDate(4, 21, 2017);
        $times = $this->salat->getPrayTime();
        ...
        $this->assertEquals(
            $expected_times,
            $times,
            'The return prayres times must match the expected pryers time for 04/21/2017'
        );
    }
    ...
    {
        $excpeted_degree_unit_scale = "39.825555555556";
        ...
        $this->assertEquals(
            $degree_unit_scale,
            $excpeted_degree_unit_scale,
            "The coveted coordinate must be {$excpeted_degree_unit_scale},
